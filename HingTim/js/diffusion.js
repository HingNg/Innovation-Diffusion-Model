...
/*
		status: 
				0 awareness  警惕                     green
				1 interest   产生兴趣                 blue
				2 evaluation 评估，to or not to 
				3 trial     尝试                      黄色
				4 adaption  采纳                      red
*/
...
var colors = d3.scale.category20();
		...
		$(document).ready(function(){
			...
			d3.json('wu.json',function(error,data){
				if (error){
					console.log(error)
				}
				dataset = data
               ...
               // .text(function (d) { return d.id})
								...
								.style("fill", function(d, i) {
									if(d.status == 4)
										return  '#F00'//colors(4)         // '#F00'
									else
										return '#004000';
										//return '#000'  //默认除中心点外，其他都是为采纳状态，为绿色
								})
								.attr("r",function(d)
								{
										if(d.id==0)
